...
- [ ] T038 [P] [US2] [RUNNABLE_NOW] **Corrective closure partial**: PostgreSQL scoped search/filter/paging, explicit target-Source Simulator duplication, activation/mutation malformed-JSON handling, and public command-seam lifecycle coverage are green; the authenticated hosted HTTP matrix and approved browser journey remain unverified in `tests/Integration/OperationalWorkspace/ConfigurationManagementTests.cs`
...
- [x] T043 [US2] [RUNNABLE_NOW] **Corrective closure**: Persist Acquisition-owned Draft-version receipts for relationship review and exact-payload validation, reject stale/missing receipts at activation, and remove client authority booleans in `src/Modules/Acquisition/Application/SimulatorConfiguration.cs`, `src/Modules/Acquisition/Infrastructure/PostgresConfigurationRepository.cs`, and management UI
...
- [x] T046 [US2] [RUNNABLE_NOW] **Corrective closure**: Implement Sites, Areas, Assets, Points, Sources, Mappings, and Simulator Configuration pages with real create/edit/detail/validate/duplicate/relationship-review/legal-lifecycle actions, explicit Site/Source/Point selectors, and target-Source duplicate selection in `src/Web/src/features/configuration/ConfigurationManagementRoutes.tsx`
...
- [ ] T048 [US2] [RUNNABLE_NOW] **Corrective closure partial**: Verification and review artifacts record receipt, selector, malformed-JSON, hosted health, and harness evidence; authenticated hosted HTTP lifecycle/replay/Audit-outbox and browser evidence remain blocked by missing app-session capability/credentials in `specs/003-operational-configuration-workspace/checklists/phase-02-checkpoint.md`
