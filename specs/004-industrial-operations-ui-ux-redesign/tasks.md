...
- [ ] T001 [RUNNABLE_NOW] Phase 0 — Correct the attempted Fast harness evidence in `specs/004-industrial-operations-ui-ux-redesign/plan.md` and `checklists/phase-00-planning-checkpoint.md` to record the actual `FAIL=1 (PASS=10)` caused by API/Worker DLL locks, with no process termination; Depends: none; Verify: `rg` shows `FAIL=1`, the lock explanation, and no reclassification as PASS or NOT_RUN.
- [ ] T002 [RUNNABLE_NOW] Phase 0 — Reconcile the permission landing flow in `research.md`, `contracts/route-and-permission-matrix.md`, `plan.md`, and the Phase 0 checkpoint as valid permitted deep link → first permitted capability → permitted Dashboard fallback → safe no-authorized-capability state; Depends: T001; Verify: route matrix and Mermaid flow never send an unauthorized user to Dashboard or expose capability/object metadata.
- [ ] T003 [RUNNABLE_NOW] Phase 0 — Append post-planning integration truth and the historical statement that the planning invocation itself did not merge in `checklists/phase-00-planning-checkpoint.md`; Depends: T001, T002; Verify: both planning commit SHAs `df73c2893fa6b936120ff5742000d0b25e180e4c` and `26b7443f7f88cb84dfbaf2fb3ffddaa6c32fcbd3` are recorded as integrated into `main`.
- [ ] T004 [P] [RUNNABLE_NOW] Phase 0 — Record the actual UI UX Pro Max supporting search and rejected landing-page/oversized-typography/package recommendations in `checklists/phase-00-planning-checkpoint.md`; Depends: T003; Verify: the command, output use, and DOC-08/dependency-policy reconciliation are written without creating a competing design source.
- [ ] T005 [P] [RUNNABLE_NOW] Phase 0 — Recheck constitution impact, Feature 003 preservation, no-backend/no-package/no-Docker boundaries, No Data semantics, and color-independent status in `plan.md` and `checklists/phase-00-planning-checkpoint.md`; Depends: T003; Verify: constitution check records PASS or an explicit non-blocking finding with path evidence.
- [ ] T006 [P] [RUNNABLE_NOW] Phase 0 — Build the FR-001–FR-028, SC-001–SC-016, C-01–C-17, route, state, and phase-stop traceability tables in `tasks.md`; Depends: T004, T005; Verify: every listed requirement, criterion, contract, route, and non-happy-path state maps to one or more task IDs.
- [ ] T007 [RUNNABLE_NOW] Phase 0 — Validate task IDs, classifications, dependency ordering, parallel markers, phase boundaries, and absence of cycles in `tasks.md`; Depends: T006; Verify: a read-only script reports 70 unique sequential IDs, no forward dependency, no duplicate ID, and no cycle.
- [ ] T008 [RUNNABLE_NOW] Phase 0 — Perform the Standards review against repository guidance and constitution in `checklists/phase-00-planning-checkpoint.md`; Depends: T001, T002, T003, T004, T005, T006, T007; Verify: review records no unresolved Critical/High governance contradiction.
- [ ] T009 [RUNNABLE_NOW] Phase 0 — Perform the Specification review against `spec.md`, `plan.md`, contracts, and locked clarifications in `checklists/phase-00-planning-checkpoint.md`; Depends: T001, T002, T003, T004, T005, T006, T007; Verify: FR/SC/contract/scope coverage records no unresolved Critical/High mismatch.
- [ ] T010 [RUNNABLE_NOW] Phase 0 — Record the final governance checkpoint and explicit stop in `checklists/phase-00-planning-checkpoint.md`; Depends: T008, T009; Verify: PASS/FAIL/BLOCKED/NOT_RUN counts, capability completeness, Planning-ready/Implementation-ready/Release-ready decisions, and the next command are explicit; no production path is changed.
...
- [ ] T028 [P] [US2] [US5] [RUNNABLE_NOW] Phase 2 — Add red evidence for dashboard exceptions, source health, coverage/freshness, zero-versus-No Data, quality states, missing chart gaps, and textual chart alternatives in `src/Web/src/test/`; Depends: T027; Verify: absent behavior fails for the requested reason before implementation.
...
- [ ] T037 [P] [US3] [RUNNABLE_NOW] Phase 3 — Add red evidence for entity lists, search/filter/sort, lifecycle/dependency states, Draft validation, first-invalid focus, unsaved changes, conflicts, and destructive confirmation in `src/Web/src/test/`; Depends: T036; Verify: absent behavior fails for the requested reason before implementation.
...
- [ ] T048 [P] [US4] [US6] [RUNNABLE_NOW] Phase 4 — Add red evidence for Simulator context/run state/history/outcomes and Audit filters/detail/redaction/deep links in `src/Web/src/test/`; Depends: T047; Verify: absent behavior fails for the requested reason before implementation.
...
- [ ] T062 [RUNNABLE_NOW] Phase 5 — Run Feature 003 regression and included-route acceptance checks using existing repository harness/test sources in `checklists/phase-05-acceptance.md`; Depends: T061, T053; Verify: permitted workflows remain reachable, backend contracts/authorization/PostgreSQL behavior are unchanged, and blocked evidence remains blocked.
...
- [ ] T064 [RUNNABLE_NOW] Phase 5 — Run mandatory Full verification against the approved repository environment and `127.0.0.1:5433/iump_dev` where the harness requires database evidence, without port 5432 or substitute stores, and record results in `checklists/phase-05-verification.md`; Depends: T063; Verify: Full evidence records exact PASS/FAIL/BLOCKED/NOT_RUN and never reclassifies a blocked prerequisite as PASS.
- [ ] T065 [BLOCKED_BY_PACKAGE_POLICY] Phase 5 — Record browser/automated accessibility evidence for the unavailable runner or axe/Playwright capability in `checklists/phase-05-verification.md` without installing packages; Depends: T063; Verify: blocker is classified `BLOCKED_BY_PACKAGE_POLICY` and remains an evidence leaf that does not block runnable review or traceability tasks.
- [ ] T066 [RUNNABLE_NOW] Phase 5 — Run architecture, repository-policy, scope, secret, dependency, and no-production-source checks in `checklists/phase-05-checkpoint.md`; Depends: T063, T064; References/Inspect: T065; Verify: no backend/API/database/package/lockfile/secret/generated-binary change and no out-of-scope capability is present.
- [ ] T067 [RUNNABLE_NOW] Phase 5 — Complete final FR/SC, user-story, C-01–C-17, route, state, and acceptance traceability in `checklists/phase-05-acceptance.md`; Depends: T062, T063, T066; References/Inspect: T064, T065; Verify: every FR-001–FR-028, SC-001–SC-016, component contract, included route, and non-happy-path state has evidence or an honest blocker.
- [ ] T068 [RUNNABLE_NOW] Phase 5 — Perform the final Standards review in `checklists/phase-05-review.md`; Depends: T067; Verify: no unresolved Critical/High standards finding and all repository policy constraints are reviewed.
- [ ] T069 [RUNNABLE_NOW] Phase 5 — Perform the final Specification review in `checklists/phase-05-review.md`; Depends: T067; Verify: no unresolved Critical/High spec mismatch and all locked clarifications remain represented.
- [ ] T070 [RUNNABLE_NOW] Phase 5 — Record the final release checkpoint and explicit stop in `checklists/phase-05-checkpoint.md`; Depends: T068, T069; Verify: PASS/FAIL/BLOCKED/NOT_RUN counts, capability completeness, Planning/Implementation/Release readiness, next command, and explicit stop are recorded; Release-ready remains NO while mandatory evidence is blocked or NOT_RUN.

**Phase 5 stop**: `/speckit.implement` for Phase 5 stops after T070.
...
T010 → Phase 1 T011–T027 → Phase 2 T028–T036 → Phase 3 T037–T047
      → Phase 4 T048–T057 → Phase 5 T058–T070
```
...
- T065 is a blocked evidence leaf; it is referenced by review/checkpoint work but is not a
...
| FR-022–FR-023 | T002, T022, T066–T067 |
| FR-024 | T017, T050, T062–T064 |
| FR-026 | T002, T005, T066 |
| FR-027 | T018, T038, T049, T051, T061 |
| FR-028 | T002, T022, T067 |
...
| SC-002 | T029, T049–T050, T062, T067 |
| SC-003 | T030–T031, T060, T067 |
| SC-004 | T017, T032, T043, T050, T067 |
| SC-005 | T011, T014, T018–T021, T059 |
| SC-006 | T014, T018, T023, T058 |
| SC-007 | T019–T020, T042, T059 |
| SC-008 | T002, T022, T051–T052, T066–T067 |
| SC-009 | T025–T026, T034–T035, T045–T046, T055–T056, T061, T068–T069 |
| SC-010 | T002, T022, T050, T066–T067 |
| SC-011 | T015, T029–T032, T067 |
| SC-012 | T053, T062, T064, T067 |
| SC-013 | T018, T038, T049, T051, T061, T067 |
| SC-014 | T014, T021, T058–T059 |
| SC-015 | T002, T022, T052, T067 |
| SC-016 | T058, T062, T064–T067 |
...
| C-09 Non-happy-path state set | T017, T032, T043, T050, T067 |
...
| C-17 ChartContainer/text alternative | T031, T060, T067 |

### Included route coverage

| Route | Migration tasks | Regression/verification |
|---|---|---|
| `/` landing | T022 | T011, T024, T062, T067 |
| `/setup` | T023 | T024, T062, T067 |
| `/dashboard` | T023, T029, T032 | T033, T062, T067 |
| `/telemetry` | T023, T030–T032 | T033, T062, T067 |
| `/configuration` and seven entities | T023, T038–T043 | T044, T062, T067 |
| `/simulator` | T023, T049–T050 | T053–T054, T062, T067 |
| `/audit` | T023, T051–T052 | T053–T054, T062, T067 |
...
verified by T017, T022, T032, T041–T043, T050, T052, T058–T060, and T067.

## Review and evidence rules

- Every phase has separate Standards and Specification review tasks followed by an explicit
  checkpoint and stop.
