...
public static class ConfigurationTests
{
    ...
    private static async Task OneHeadAndImmutableVersions(List<string> failures)
    {
        ...
        var first = Version(configurationId, 1, 60, 5, 5, "seed-constant", SimulatorScenario.Constant, "creator", "creator@example", "corr-1", "caus-1");
        ...
        var second = Version(configurationId, 2, 30, 1, 3, "stable-seed", SimulatorScenario.Normal, "editor", "editor@example", "corr-2", "caus-2");
        ...
        Assert((await repo.GetVersionAsync(configurationId, 2))?.DeterministicSeed == "stable-seed", failures, "Exact immutable version lookup works.");
    ...
    {
        var invalid = false;
        try { _ = Version(Guid.NewGuid(), 1, 10, 2, 3, "seed", SimulatorScenario.Constant, "u", "user", null, null); }
        catch (ArgumentException) { invalid = true; }
        Assert(invalid, failures, "Constant scenario requires equal bounds.");
        invalid = false;
        try { _ = Version(Guid.NewGuid(), 1, 10, 4, 4, "seed", SimulatorScenario.Normal, "u", "user", null, null); }
        catch (ArgumentException) { invalid = true; }
        Assert(invalid, failures, "Normal scenario requires minimum below maximum.");
        invalid = false;
        try { _ = Version(Guid.NewGuid(), 1, 0, 4, 4, "seed", SimulatorScenario.Constant, "u", "user", null, null); }
        ...
        var first = Version(configurationId, 1, 10, 1, 1, "seed", SimulatorScenario.Constant, "u", "user", null, null);
        ...
        var stale = false;
        try { await repo.AppendVersionAsync(configurationId, 99, Version(configurationId, 2, 10, 2, 2, "seed-2", SimulatorScenario.Constant, "u", "user", null, null)); }
    ...
    internal static SimulatorConfigurationVersion Version(Guid configurationId, long version, int interval,
        double min, double max, string seed, SimulatorScenario scenario, string userId, string username,
