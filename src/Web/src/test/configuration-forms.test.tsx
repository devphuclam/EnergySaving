...
export function runConfigurationFormChecks(): string[] {
  ...
  if (configurationValidationErrors('sites', 'create', {})[0]?.label !== 'Tên') failures.push('validation errors must carry Vietnamese labels, never property keys')
  const pointIdErrors = configurationValidationErrors('points', 'edit', {})
  if (pointIdErrors.some(error => error.key === 'metricId' && error.label === 'metricId')) failures.push('metricId errors must carry the Vietnamese label Chỉ số, never the property key')
  if (pointIdErrors.some(error => error.key === 'unitId' && error.label === 'unitId')) failures.push('unitId errors must carry the Vietnamese label Đơn vị, never the property key')
  if (pointIdErrors.some(error => error.key === 'dataOwnerUserId' && error.label === 'dataOwnerUserId')) failures.push('dataOwnerUserId errors must carry the Vietnamese label Chủ dữ liệu, never the property key')
  ...
  if (!zeroInterval.errors.some(error => error.key === 'expectedIntervalSeconds')) failures.push('zero must be rejected for a positive interval field per the server domain rule')
  if ('expectedIntervalSeconds' in zeroInterval.body) failures.push('a rejected interval must never be transmitted as zero')
  if (!normalizeConfigurationForm('points', 'edit', { expectedIntervalSeconds: 'Infinity' }).errors.some(error => error.key === 'expectedIntervalSeconds')) failures.push('Infinity must be rejected')
  const minMax = normalizeConfigurationForm('simulator-configurations', 'edit', { minimumValue: '50', maximumValue: '20' })
  if (!minMax.errors.some(error => error.key === 'minimumValue')) failures.push('minimum above maximum must be rejected')
  if (normalizeConfigurationForm('points', 'edit', { metricId: 'm-1', unitId: 'u-1', dataOwnerUserId: 'u-1', expectedIntervalSeconds: '30', noDataAfterSeconds: '' }).errors.length) failures.push('valid Point technical IDs with an empty optional numeric must not block a valid submit')
