...
export function dashboardExceptionItems(snapshot: OperationalDashboardSnapshot): DashboardException[] {
  ...
  for (const [index, item] of snapshot.health.items.slice(0, 8).entries()) {
    ...
    if (status !== 'Online') items.push({
      key: `health-${index}`, title: `Sức khỏe nguồn: ${pointLabel(snapshot, item)}`,
  ...
  for (const [index, item] of snapshot.latest.items.slice(0, 8).entries()) {
    ...
    if (quality === 'Bad' || quality === 'Uncertain' || quality === 'Missing') items.push({
      key: `quality-${index}`, title: `Chất lượng dữ liệu: ${pointLabel(snapshot, item)}`,
...
function DashboardReady({ snapshot, onNewSetup, onContinueSetup, onNavigate }: { snapshot: OperationalDashboardSnapshot; onNewSetup: () => void; onContinueSetup: () => void; onNavigate: (route: DashboardRoute) => void }) {
  ...
  return <>
    ...
    <div className="dashboard-evidence-grid"><section className="evidence-panel" aria-labelledby="dashboard-health-title"><div className="section-heading"><div><p className="card-kicker">Source health</p><h2 id="dashboard-health-title">Tình trạng nguồn</h2></div><button className="button button-secondary" type="button" onClick={() => onNavigate('telemetry')}>Mở Measurement</button></div>{snapshot.health.items.length === 0 ? <EmptyState title="Chưa có source health" message="Response hiện tại không trả bản ghi source health trong phạm vi này." /> : <ul className="evidence-list">{snapshot.health.items.slice(0, 8).map((item, index) => { const status = textOf(item, 'status', 'Status'); const presentation = dashboardHealthPresentation(status); const received = textOf(item, 'lastReceivedAtUtc', 'LastReceivedAtUtc'); return <li key={`${pointLabel(snapshot, item)}-${index}`}><strong>{pointLabel(snapshot, item)}</strong><OperationalStatusBadge status={presentation.status} detail={status ?? 'Unknown'} /><span className="metadata">{received ? `Lần nhận cuối: ${received}` : 'Lần nhận cuối: chưa có'}</span></li> })}</ul>}</section>
      <section className="evidence-panel" aria-labelledby="dashboard-quality-title"><div className="section-heading"><div><p className="card-kicker">Data quality</p><h2 id="dashboard-quality-title">Chất lượng giá trị mới nhất</h2></div><span className="metadata">{DASHBOARD_QUALITY_REASON_UNAVAILABLE}</span></div>{snapshot.latest.items.length === 0 ? <EmptyState title="Chưa có giá trị mới nhất" message="Không có bản ghi hiện tại trong response; đây không phải giá trị zero." /> : <ul className="evidence-list">{snapshot.latest.items.slice(0, 8).map((item, index) => { const quality = qualityOf(valueOf(item, 'quality', 'Quality')); return <li key={`${pointLabel(snapshot, item)}-${index}`}><strong>{pointLabel(snapshot, item)}</strong>{quality ? <DataQualityIndicator quality={quality} /> : <OperationalStatusBadge status="Unavailable" detail="Quality chưa có" />}<span className="numeric">{String(valueOf(item, 'value', 'Value') ?? 'Missing')} {String(valueOf(item, 'unit', 'Unit') ?? '')}</span></li> })}</ul>}</section></div>
