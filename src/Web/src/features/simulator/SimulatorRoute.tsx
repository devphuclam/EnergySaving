...
export function SimulatorRoute() {
  ...
  return (
    <section className="page" aria-labelledby="simulator-title">
      ...
      {(snapshot.errorCode || snapshot.isReplay) && <div className="notice notice-warning" role="status">{snapshot.isReplay ? 'Mutation replayed.' : `Mutation feedback: ${snapshot.errorCode}`}</div>}
