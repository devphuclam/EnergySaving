...
const empty: ConfigurationSummary = { state: 'loading', siteCount: 0, areaCount: 0, pointCount: 0 }
...
export function ConfigurationRoutes() {
  ...
  return (
    <section className="page" aria-labelledby="configuration-title">
      ...
      {unavailable ? <div className="notice notice-warning" role="status">Configuration gateway: {summary.state}. No local fallback data is shown.</div> : <div className="card-grid three-up"><article className="card"><p className="card-kicker">Hierarchy</p><h2>{summary.siteCount} sites</h2><p className="muted">Server-scoped Site → Area → Asset → Measurement Point.</p><button className="button button-secondary" type="button">Open hierarchy</button></article><article className="card"><p className="card-kicker">Catalog</p><h2>{summary.areaCount} areas</h2><p className="muted">Catalog and source mappings are read from the gateway.</p><button className="button button-secondary" type="button">Review mapping</button></article><article className="card"><p className="card-kicker">Points</p><h2>{summary.pointCount} points</h2><p className="muted">Versioned configuration remains Draft until activation.</p><button className="button button-secondary" type="button" onClick={() => void validate()}>Validate configuration</button></article></div>}
